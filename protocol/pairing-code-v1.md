...
- タイムアウト（既定: 接続待ち 120s、接続後の各ステップ 30s。直接入力の confirm 待ちだけは
  v1.2 の SAS 有効期限 60s）→ abort。
- 同時に受け付ける接続は 1 本。v1.2 から abort 後も待受は継続するが、**1 回の setup で受理する接続は
  上限 5 回**（総待受 600s 内）。上限は「単発試行」の意図（MITM に online 総当りを与えない）を保つためで、
  MITM の成功確率は ≤ 5 × 2^-20（詳細は v1.2 節）。
...
## v1.2 拡張 — SAS 有効期限（`server_key.ttl`）と再発行

v1.1 までの直接入力は、責任者不在のまま「コード表示から 30s（各ステップ共通の timeout）以内に
6 桁を打つ」必要があり、リモート VM（ターミナルでコードを読んで iPhone に持ち替える）では
間に合わずに abort → setup 再実行、という詰まり方をした。v1.2 は次の 3 点を **additive** に足す
（凍結 salt / info / メッセージ列は不変。旧アプリ・旧ホストとは互換マトリクスどおり相互運用）。

### `server_key.ttl`（追加フィールド）

```
{ "t":"server_key", "v":1, "epk":"...", "ck":1, "ttl":60 }
```

- `ttl` — SAS（6 桁コード）の有効期限（**秒**、正の整数）。responder は **直接入力（`hello.psk` なし）
  のときだけ**載せる。PSK モードには人間照合が無いので載せない。
- 既定 60s（host-ts `DEFAULT_SAS_TTL_MS`）。責任者は responder: SAS 表示から `ttl` 経過で confirm が
  無ければ abort（理由 `sas expired`）して接続を閉じる。
- initiator は表示にのみ使う（残り時間のカウントダウン）。旧ホストは載せないので `nil`＝表示なし。
  不正値（0 以下・非整数・文字列）は両実装とも「未通知」として落とす。

### 期限切れの検知と再発行（= 再接続）

- SAS は共有鍵から導くため「同じ接続で別のコードを配る」ことはできない。**再発行 = 同じ待受へ
  再接続して鍵交換をやり直すこと**。
- initiator は 6 桁入力待ちの間もストリームのクローズを見張り、閉じたら入力を待たずに
  `expired` で終える（見張らないと「確認」を押して初めて失敗し、理由も timeout に化ける）。
  UI は「コードを再発行」（同一 host:port へ再接続）を出す。自動再接続はしない（入力中にコードが
  差し替わる事故を避ける）。
- responder（setup）は abort 後も待受を閉じず、**総待受 600s** の間は次の接続を受け続ける。
  成立（paired）で終了。待受中・SAS 表示中はターミナルに残り時間を毎秒表示する。
- abort 理由は `closed`（相手が切断）/ `timeout`（ステップ無応答）/ `sas expired`（SAS 期限切れ）を
  区別する。initiator 側のキャンセルを「期限切れ」と誤案内しないため。

### 試行回数の上限（待受継続と「単発試行」の両立）

待受を継続すると、能動 MITM がホスト側で鍵交換だけを高速に反復できる: MITM は `server_key` を受けた
時点で（`reveal` を送る前に）自分と responder の共有鍵 Z₂ と SAS₂ を計算できるので、iPhone 側に
表示中の SAS₁ と衝突する鍵を 1 接続 = RTT 1 回で探索でき、600s で 10^5 回規模 → 成功確率が
2^-20 から数 % に跳ね上がる。よって responder は **1 回の setup で受理する接続を 5 回まで**とし
（`hello` の成否を問わず接続ごとに数える）、超えたら終了して再実行を促す。MITM の成功確率は
≤ 5 × 2^-20 ≈ 5×10^-6 で v1 の単発試行と同等、人間の再発行 2〜3 回は収まる。

### 接続失敗の分類（initiator）

`NWConnection` は拒否・到達不能で自発的に失敗せず `.waiting` に留まる。v1.2 の initiator は
- `ECONNREFUSED`（到達したが待受が無い＝setup 終了済み）→ 即失敗。「setup を実行し直す」を案内
  （QR 経路なら「QR は期限切れ」）。
- それ以外の `.waiting`（ICMP 不達・DNS 失敗）→ タイムアウト（120s）まで待って「ホストに到達できません。」
- `.waiting` を一度も見ないタイムアウト → 従来の汎用文言。
