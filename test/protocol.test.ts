...
describe("golden roundtrip", () => {
  ...
  it("file-download v1 golden 全行が byte-exact でラウンドトリップする", () => {
    for (const line of goldenLines("file-download-v1.ndjson")) {
      const decoded = decodeControlMessage(line);
      expect(encodeControlMessage(decoded)).toBe(line);
    }
  });
  ...
  it("account-usage v1 golden 全行が byte-exact でラウンドトリップする", () => {
    for (const line of goldenLines("account-usage-v1.ndjson")) {
      expect(encodeControlMessage(decodeControlMessage(line))).toBe(line);
    }
  });
  ...
  it("pane-key v1 golden 全行が byte-exact でラウンドトリップする", () => {
    for (const line of goldenLines("pane-key-v1.ndjson")) {
      const decoded = decodeControlMessage(line);
      expect(encodeControlMessage(decoded)).toBe(line);
    }
  });
  ...
  it("host-name v1 golden 全行が byte-exact でラウンドトリップする", () => {
    for (const line of goldenLines("host-name-v1.ndjson")) {
      const decoded = decodeControlMessage(line);
      expect(encodeControlMessage(decoded)).toBe(line);
    }
  });
